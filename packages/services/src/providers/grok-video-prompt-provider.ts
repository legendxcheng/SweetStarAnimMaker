  ...
  return {
    async generateVideoPrompt(input: GenerateVideoPromptInput): Promise<GenerateVideoPromptResult> {
      ...
      const rawText = await requestOpenAiCompatibleChatCompletion({
        ...
        systemText: [
          "You generate structured JSON video prompt plans for Kling Omni single-shot video generation.",
          "Output must be valid JSON.",
          "finalPrompt must be Simplified Chinese natural-language prompt text for Kling Omni.",
          "Preserve provided dialogue, narration, audio, and continuity constraints.",

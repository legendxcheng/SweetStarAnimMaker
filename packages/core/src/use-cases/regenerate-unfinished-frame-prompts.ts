...
export interface RegenerateUnfinishedFramePromptsInput {
  projectId: string;
}

export interface RegenerateUnfinishedFramePromptsResult {
  batchId: string;
  frameCount: number;
  taskIds: string[];
}

export interface RegenerateUnfinishedFramePromptsUseCase {
  execute(
    input: RegenerateUnfinishedFramePromptsInput,
  ): Promise<RegenerateUnfinishedFramePromptsResult>;
}

export interface RegenerateUnfinishedFramePromptsUseCaseDependencies {
...
export function createRegenerateUnfinishedFramePromptsUseCase(
  dependencies: RegenerateUnfinishedFramePromptsUseCaseDependencies,
): RegenerateUnfinishedFramePromptsUseCase {
  return {
    async execute(input) {
      ...
      return {
        ...
        frameCount: unfinishedFrames.length,
        taskIds,
      };
    },
  };
...
function isFramePromptUnfinished(frame: { planStatus: string; promptTextCurrent?: string | null }) {
  if (frame.planStatus !== "planned") {
    return true;
  }

  return (frame.promptTextCurrent ?? "").trim().length === 0;
}

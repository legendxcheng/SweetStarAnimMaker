...
import { createUpdateVideoPromptUseCase, SegmentVideoNotFoundError } from "../src/index";
...
describe("update video prompt use case", () => {
  ...
  it("revokes approval for the legacy prompt alias and re-derives project status", async () => {
  ...
  it("rejects prompt edits for a segment outside the current video batch", async () => {
    const projectRepository = {
      insert: vi.fn(),
      findById: vi.fn().mockResolvedValue({
        id: "proj_1",

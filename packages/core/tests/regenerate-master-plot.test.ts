...
import { createRegenerateMasterPlotUseCase } from "../src/index";

describe("regenerate master plot use case", () => {
  it("resets downstream stage pointers and restarts master plot generation", async () => {
    const createMasterPlotGenerateTask = {
      execute: vi.fn().mockResolvedValue({
        id: "task_master_plot_restart",
      }),
    };
    ...
    const useCase = createRegenerateMasterPlotUseCase({
      projectRepository,
    ...
    expect(createMasterPlotGenerateTask.execute).toHaveBeenCalledWith({
      projectId: "proj_20260324_ab12cd",
    });
    expect(result.id).toBe("task_master_plot_restart");
  });
});

...
describe("process segment video generate task use case", () => {
  ...
  it("keeps the project in videos_generating when another segment in the batch is still generating after success", async () => {
    const task = {
      id: "task_segment_video_1",
      projectId: "proj_1",
      type: "segment_video_generate",
    ...
    expect(videoRepository.listSegmentsByBatchId).toHaveBeenCalledWith("video_batch_v1");
    expect(projectRepository.updateStatus).toHaveBeenCalledWith({
      projectId: "proj_1",

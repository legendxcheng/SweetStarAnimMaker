...
describe("process master plot generate task use case", () => {
  ...
  it("drops provider results when the task is reset before persistence", async () => {
    const task = {
      id: "task_20260322_master_plot",
      projectId: "proj_20260322_ab12cd",
    ...
    const taskRepository = {
      insert: vi.fn(),
      findById: vi.fn().mockResolvedValueOnce(task).mockResolvedValueOnce(null),
      findLatestByProjectId: vi.fn(),
      delete: vi.fn(),
      markRunning: vi.fn(),
      markSucceeded: vi.fn(),
    ...
    await expect(useCase.execute({ taskId: "task_20260322_master_plot" })).resolves.toBeUndefined();

    expect(masterPlotStorage.writeRawResponse).not.toHaveBeenCalled();
    expect(masterPlotStorage.writeCurrentMasterPlot).not.toHaveBeenCalled();
    ...
    expect(taskRepository.markSucceeded).not.toHaveBeenCalled();
    expect(taskRepository.markFailed).not.toHaveBeenCalled();
  });

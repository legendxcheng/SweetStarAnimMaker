  ...
  it("restores an existing frame image to in review when regeneration fails", async () => {
    ...
    const shotImageProvider = {
      generateShotImage: vi
        .fn()
        .mockRejectedValue(new Error("Turnaround image provider returned no usable image")),
    };
    ...
    await expect(useCase.execute({ taskId: "task_frame_image_1" })).rejects.toThrow(
      "Turnaround image provider returned no usable image",
    );
    ...
    expect(taskRepository.markFailed).toHaveBeenCalledWith({
      taskId: "task_frame_image_1",
      errorMessage: "Turnaround image provider returned no usable image",
      updatedAt: "2026-03-24T00:18:00.000Z",
      finishedAt: "2026-03-24T00:18:00.000Z",
    });
  });
